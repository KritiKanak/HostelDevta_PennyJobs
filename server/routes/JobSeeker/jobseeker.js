...
router.post('/createuser',[
    ...
    try{
        ...
        if(user){
            return res.status(400).json({success, error:" Jobseeker with this email already exists"})
        } 
...
//Route 2: Authenticate a Jobseeker using: POst "/api/auth/login" , NO Login Required
router.post('/login',[
    // body('name','Enter a Valid Name').isLength({ min: 3 }),
    body('email','Enter a valid email').isEmail(),
...
// Route 3: Get loggedin Jobseeker details using: POST "/api/auth/getuser". Login required
